...
public class AmplitudeModule
     ...
     * @param userPropsString JSON string with user properties to be set.
     */
    @ReactMethod
    public void setUserProperties(String instanceName, String userPropsString) {
        JSONObject userProps = null;
        try {
            userProps = new JSONObject(userPropsString);
        } catch (JSONException e) {
            e.printStackTrace();
        }

        Amplitude.getInstance(instanceName).setUserProperties(userProps);
    }
